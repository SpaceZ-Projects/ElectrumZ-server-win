...
;Variables
  ...
  !define PRODUCT_VERSION "1.16.1"
  !define PRODUCT_NAME "ElectrumZ-Server"
  !define PRODUCT_WEB_SITE "https://github.com/SpaceZ-Projects/ElectrumZ-server-win"
...
;General
  ...
  ;Name and file
  Name "${PRODUCT_NAME}"
  OutFile "dist/electrumz-server-${PRODUCT_VERSION}-setup-${BUILD_ARCH}.exe"
...
;Interface Settings
  ...
  !define MUI_ICON "icons\electrumz.ico"
...
Section
  ...
  File /r "dist\electrumz-server\*.*"
  File "icons\electrumz.ico"
  ...
  CreateShortCut "$DESKTOP\${PRODUCT_NAME}.lnk" "$INSTDIR\ElectrumZ-Server.exe" ""

  ;Create start-menu items
  DetailPrint "Creating start-menu items..."
  CreateDirectory "$SMPROGRAMS\${PRODUCT_NAME}"
  CreateShortCut "$SMPROGRAMS\${PRODUCT_NAME}\Uninstall.lnk" "$INSTDIR\Uninstall.exe" "" "$INSTDIR\Uninstall.exe" 0
  CreateShortCut "$SMPROGRAMS\${PRODUCT_NAME}\${PRODUCT_NAME}.lnk" "$INSTDIR\ElectrumZ-Server.exe" "" "$INSTDIR\ElectrumZ-Server.exe" 0
  ...
  WriteRegStr HKCU "Software\Classes\btcz" "DefaultIcon" "$\"$INSTDIR\electrumz.ico, 0$\""
  WriteRegStr HKCU "Software\Classes\btcz\shell\open\command" "" "$\"$INSTDIR\ElectrumZ-Server.exe$\" $\"%1$\""
  ...
  WriteRegStr HKCU "${PRODUCT_UNINST_KEY}" "DisplayIcon" "$INSTDIR\electrumz.ico"
